...
class RandomShortestSize(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.min_size = (
            [min_size] if isinstance(min_size, int) else list(min_size)
        )
...
def _copy_paste(
    ...
    if size1 != size2:
        ...
        paste_boxes = (
            paste_boxes.view(-1, 2, 2).mul(ratios).view(paste_boxes.shape)
        )
    ...
    if degenerate_boxes.any():
        ...
        if "iscrowd" in out_target and len(out_target["iscrowd"]) == len(
            valid_targets
        ):
